...
use apw_core::config::{
    BarkAlertMode, ConfigError, DEFAULT_INTERVAL_SECONDS, MAX_SETTINGS_BYTES, Settings,
...
fn 样例设置() -> Settings {
    let targets = vec![目标("R683", "MG724CH/A"), 目标("R448", "MG0A4CH/A")];
    Settings {
        locale: "zh_CN".into(),
...
fn 迁移能读出旧版格式() {
    ...
    assert_eq!(got.users[0].bark_url, "https://api.day.app/legacy");
    assert_eq!(got.users[0].targets, got.targets);
...
fn 设置的线上格式是小驼峰() {
    ...
    assert_eq!(obj.len(), 7);
    assert_eq!(obj["users"][0]["alertMode"], "critical");
}
