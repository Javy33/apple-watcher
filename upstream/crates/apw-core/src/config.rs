...
impl Default for UserSettings {
    fn default() -> Self {
        Self {
            ...
            name: String::new(),
            bark_url: String::new(),
            alert_mode: BarkAlertMode::Active,
            targets: Vec::new(),
        }
    }
}
...
impl Settings {
    ...
    pub fn normalize(&mut self) {
        ...
        // 旧版的全局目标和 Bark 无损迁移成一个默认用户。旧字段保留读取
        // 兼容，新界面只写 users。
        if self.users.is_empty() && (!self.targets.is_empty() || !self.bark_url.trim().is_empty()) {
            self.users.push(UserSettings {
                id: "legacy".into(),
                name: "默认用户".into(),
                bark_url: self.bark_url.clone(),
                alert_mode: BarkAlertMode::Active,
                targets: self.targets.clone(),
            });
        }

        let mut ids = std::collections::HashSet::with_capacity(self.users.len());
        for (index, user) in self.users.iter_mut().enumerate() {
            user.id = user.id.trim().to_string();
            if user.id.is_empty() || !ids.insert(user.id.clone()) {
                user.id = format!("user-{}", index + 1);
                while !ids.insert(user.id.clone()) {
                    user.id.push('-');
                }
            }
            user.name = user.name.trim().to_string();
            if user.name.is_empty() {
                user.name = format!("用户 {}", index + 1);
            }
            user.bark_url = user.bark_url.trim().to_string();
            user.targets = normalize_targets(&user.targets);
        }
    }
    ...
    /// 所有用户的监控目标去重后交给同一个查询引擎。
    pub fn watch_targets(&self) -> Vec<Target> {
        let mut seen = std::collections::HashSet::new();
        self.users
            .iter()
            .flat_map(|user| &user.targets)
...
fn normalize_targets(targets: &[Target]) -> Vec<Target> {
    let mut seen = std::collections::HashSet::with_capacity(targets.len());
    targets
        .iter()
        .filter(|target| {
            !target.locale.trim().is_empty()
                && !target.store_number.trim().is_empty()
                && !target.part_number.trim().is_empty()
...
mod tests {
    ...
    #[test]
    fn 多用户的相同目标只查一次() {
        let shared = 目标("R683", "MG724CH/A");
        let mut settings = Settings {
            users: vec![
                UserSettings {
                    id: "a".into(),
                    name: "A".into(),
                    targets: vec![shared.clone()],
                    ..UserSettings::default()
                },
                UserSettings {
                    id: "b".into(),
                    name: "B".into(),
                    targets: vec![shared.clone(), 目标("R448", "MG0A4CH/A")],
                    ..UserSettings::default()
                },
            ],
            ..Settings::default()
